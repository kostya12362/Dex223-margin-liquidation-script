...
#     ):
#         """
#         Upsert по PK (id). Если запись есть — обновляем статусы.
#         """
...
#             set_={
#                 "is_liquidated": (
#                     insert(Position).excluded.is_liquidated
#                 ),
#                 "predict_check_timestamp": (
#                     insert(Position).excluded.predict_check_timestamp
#                 ),
#             },
#         )
...
#             update(Position)
#             .where(Position.id == position_id)
#             .values(predict_check_timestamp=ts)
#         )
#         await self.session.execute(stmt)
#         await self.session.commit()
#
...
#
#         # A) positions from a specific pool (active and not liquidated)
#         q_pool = (
#             select(Position.id.label("position_id"))
#             .where(
#                 Position.frozen_by.is_(None),
...
#         )
#
#         # B) positions with expired predict (active and not liquidated), regardless of pool
#         q_overdue = (
#             select(Position.id.label("position_id"))
#             .where(
#                 Position.frozen_by.is_(None),
